...
const RecentShelves = ({recentShelves} : any) => {
    ...
    <div className="flex flex-col gap-1 mt-1 divide-[#DBC9E5] divide-[1px]">
      ...
      <div className="flex justify-between mr-3">
        {recentShelves.map((item, index) => (
          <div className={`w-[66px] mr-3 cursor-pointer ${index === 0 ? 'shadow bg-[#DFE3EE] p-2 rounded-lg' : ''}`} key={index}>
          <div className={`${index ===0 ? 'w-12 h-12 rounded-full mb-3' : 'w-16 h-16 rounded-sm'}`} style={{background: backgroundColors[index]}}/>
